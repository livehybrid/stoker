...
- **fleets**: id, name (unique), driver (swarm/k8s/inprocess/fake), config_json (portainer endpoint id + host; or k8s `namespace` + optional `kube_context` / `in_cluster` / `node_selector`; or inprocess `max_workers` / `control_url`; or fake), version_info, last_seen_at, created_at. `fake-local` (fake), `swarm-local` (swarm) and `k8s-local` (k8s, namespace from `K8S_NAMESPACE`, default nodeSelector from `K8S_NODE_SELECTOR`) are seeded at first boot from config, plus `inprocess-local` (inprocess) when `STOKER_INPROCESS_FLEET` is set; further fleets (e.g. a second cluster) are operator-added rows.
...
- **Supervisor tick** (`lifecycle.supervisor_tick`, every ~2 s in the lifespan):
  ...
  - **target-health backpressure** (opt-in, `STOKER_BACKPRESSURE_DRAIN`): when the share of the fleet's *recent* HEC delivery attempts that failed (5xx + timeouts, per-slot last-two-samples delta) meets `STOKER_BACKPRESSURE_MIN_FAILED_FRACTION` (0.5) and holds for `STOKER_BACKPRESSURE_SUSTAINED_S` (60 s), the supervisor acts per `STOKER_BACKPRESSURE_ACTION`. **`step_down` (default)**: the run rate is multiplied by `STOKER_BACKPRESSURE_STEP_FACTOR` (0.5) and generation CONTINUES, re-apportioned and pushed to live workers as a `retarget` via the same `rescale_run` path an operator rescale uses; the run is marked `degraded` and a `backpressure_step_down` event recorded. One step per sustained window (the step resets the streak clock, so the rate cannot collapse on consecutive ticks). It escalates to a drain once the run is at the floor (`STOKER_BACKPRESSURE_MIN_RATE_FRACTION`, 0.1, of the ORIGINALLY requested rate — recorded on the snapshot at the first step so repeated halvings cannot creep below it) and still failing, or when the run is conf-paced (`count_interval`) and so has no run rate to step. **`drain`**: the run is stopped gracefully (sent data stays valid) and its target flagged red. Either way it is on the audit trail (`backpressure_detected` / `backpressure_cleared` / `backpressure_step_down` / a `backpressure-drain` state transition). A single failing tick only starts the clock; a healthy tick clears it, so a blip never acts. Disabled by default, so no run changes behaviour until an operator opts in.
...
- **K8sDriver** (`drivers/k8s.py`): drives the kubernetes client (BatchV1/CoreV1), namespace from the fleet config (default: the `K8S_NAMESPACE` setting). Auth: the fleet config's explicit choice wins (`in_cluster: true` -> the pod service account; `kube_context` -> that kubeconfig context, k3s local vs EKS driven from outside); with neither, auto-detect tries a kubeconfig first and falls back to the in-cluster service account, so a control plane deployed **inside** a cluster (an EKS pod) works with zero fleet config. An auth failure raises a DriverError naming what was tried, never a raw "Invalid kube-config file" traceback.
- **InProcessDriver** (`drivers/inprocess.py`): FakeDriver's spawn mode productized — one run == N real `stoker_agent` subprocesses of the control-plane process, speaking the normal managed contract against `http://127.0.0.1:<PORT>` (never hairpinning through `PUBLIC_BASE_URL`). For SMALL workloads: workers share the control plane's CPU/memory with no container isolation, so the worker count is capped (`STOKER_INPROCESS_MAX_WORKERS`, default 2; fleet `config_json.max_workers` overrides — enforced at the submit gate and again in `create`/`scale`) and custom-code packs are refused at submit unconditionally. Opt-in: the driver only builds under `STOKER_INPROCESS_FLEET=1` and requires the worker source in the image (`/app/worker`, shipped by `server/Dockerfile`; a repo checkout uses the sibling `worker/` tree). Each slot gets its own `STOKER_OUTPUT_SOCKET` (shared /tmp), the per-worker prometheus port is disabled (heartbeats already carry the counters), and worker stdout/stderr is captured into a bounded ring so `GET /runs/{id}/logs` works. Workers die with the control-plane process — after a restart the boot stray-sweep sees an empty in-memory fleet, matching reality. One run == one Indexed `batch/v1` Job named `stoker-run-<id>` labelled `stoker.run=<id>`. `create` writes a per-run ephemeral **Secret** carrying the HEC token (referenced via `secretKeyRef`, never inline in the pod env) then the **Job** with `completionMode: Indexed`, `parallelism == completions == N`, `backoffLimit`, and `ttlSecondsAfterFinished` as the stray-catcher; the Secret is adopted under the Job via an `ownerReference` so it is GC'd with the Job. `scale` is Elastic Indexed Jobs (k8s >= 1.27): patch `parallelism` and `completions` together. `stop`/`destroy` delete the Job with `propagationPolicy: Foreground` (the Secret GCs via its ownerRef). `status` reads the Job (desired = `spec.parallelism`) and lists pods (the Indexed-Job pod env carries each pod's completion index, which the worker reads as its slot). `logs` reads pod logs. EKS Terraform for the cluster lives under `infra/`.
...
- `GET /api/specs/{id}/export` -> `{spec, references, curl}` to reproduce the spec in CI/CD: `spec` is a minimal `SpecCreate` body (None fields dropped), `references` names the pack + target (id **and** name, for remapping across environments) and `curl` is a ready-to-run `POST /api/specs` using `$STOKER_URL` / `$STOKER_TOKEN`. No secrets (the target token stays server-side; recreate the target separately).
- `PUT /api/specs/{id}`, `DELETE /api/specs/{id}` (409 when the spec has runs).
- **Multi-pack specs.** A spec carries a primary `pack_id` plus an optional `extra_pack_ids` list (stored as the nullable `specs.extra_pack_ids_json`, added by migration `0003_spec_extra_pack_ids`). Absent/empty = the classic single-pack spec, byte-for-byte unchanged. When set, `_resolve_bundle` builds the run's bundle with `bundles.build_from_packs`: the union of every pack's stanzas and sample files, each prefixed `<namespace>--` where the namespace is the pack name sanitised to `[A-Za-z0-9_-]` (de-duplicated by pack id when two names sanitise alike). Stanza and sample filenames are renamed together, so each stanza still resolves its sample against the single flat `sampleDir` the conf rewrite stamps. Inputs are sorted by namespace before emission, so the same set of packs always yields the same digest regardless of selection order — preserving content-addressed bundle dedup. `est_bytes_per_event` is recomputed across the merged sample set. Guarded by `_multi_pack_guard` at spec create, spec update AND run submit (`422 multi_pack_engine_unsupported`): eventgen only, since rawreplay is a single dataset forced to one worker and a metrics pack has no files to merge.
...
- **Dogfood self-telemetry** (optional, enabled only when both `DOGFOOD_HEC_URL` and `DOGFOOD_HEC_TOKEN` are set): a `stoker:job` HEC event per run state transition (`run_id`, `spec_id`, `from`, `to`, `end_reason`, `degraded`), and a periodic `stoker:metrics` aggregate per active run (`dogfood_metrics_interval_s`, ~30 s): `run_id`, `spec_id`, `state`, `eps`, `bps`, `events_total`, `bytes_total`, `lag_s_max` (worst live worker's pacing lag), `live_workers`, `reporting_workers`, cumulative `hec_2xx/4xx/5xx/hec_timeouts/retries`, and `target_eps` (the run's intended eps, present only for an `eps` run so a dashboard can plot delivered-vs-target). Every dogfood path is a best-effort no-op when disabled; HEC failures are swallowed and the token is never logged. `host` on the envelope is the public base URL's host. The shippable `dashboards/stoker-observability` Splunk app renders these two sourcetypes (fleet overview + per-run drill-down).
...
- **Kubernetes**: `K8S_NAMESPACE` (default `stoker`) — the namespace the seeded `k8s-local` fleet runs worker Jobs in; per-fleet `config_json.namespace` overrides. `K8S_NODE_SELECTOR` (`key=value[,key=value...]`, default unset) — fleet-level default nodeSelector for worker Jobs, seeded into `k8s-local`'s config_json; a spec's `driver_opts.node_selector` overrides it per run. `K8S_TOLERATIONS` (`key[=value][:effect][,...]`, default unset) — fleet-level default pod tolerations for worker Jobs, seeded into `k8s-local`'s config_json, so workers pinned to a reserved node group tolerate its `NoSchedule` taint (a spec's `driver_opts.tolerations` overrides). Kube auth needs no env: kubeconfig or the pod service account is auto-detected (see K8sDriver).
...
- **Target-health backpressure**: `STOKER_BACKPRESSURE_DRAIN` (default off) — act on a run whose HEC target is failing to accept data. `STOKER_BACKPRESSURE_ACTION` (`step_down` | `drain`, default `step_down`) — halve the rate and keep generating, or stop the run. `STOKER_BACKPRESSURE_STEP_FACTOR` (default 0.5) — the multiplier per step; `STOKER_BACKPRESSURE_MIN_RATE_FRACTION` (default 0.1) — the floor as a fraction of the originally requested rate, below which a still-failing run drains. `STOKER_BACKPRESSURE_MIN_FAILED_FRACTION` (default 0.5) — the share of recent fleet delivery attempts (5xx + timeouts) that counts as the target struggling; `STOKER_BACKPRESSURE_SUSTAINED_S` (default 60) — how long that must hold before acting, and between successive steps. See the supervisor-tick backpressure bullet.
