...
- **Supervisor tick** (`lifecycle.supervisor_tick`, every ~2 s in the lifespan):
  ...
  - **target-health backpressure** (opt-in, `STOKER_BACKPRESSURE_DRAIN`): when the share of the fleet's *recent* HEC delivery attempts that failed (5xx + timeouts, per-slot last-two-samples delta) meets `STOKER_BACKPRESSURE_MIN_FAILED_FRACTION` (0.5) and holds for `STOKER_BACKPRESSURE_SUSTAINED_S` (60 s), the run is **drained** (a graceful stop — sent data stays valid) and its target flagged red, recorded on the audit trail (`backpressure_detected` / `backpressure_cleared` / a `backpressure-drain` state transition). A single failing tick only starts the clock; a healthy tick clears it, so a blip never drains. Disabled by default, so no run changes behaviour until an operator opts in. Rate step-down (vs drain) is the documented alternative, not yet implemented.
...
- **Target-health backpressure**: `STOKER_BACKPRESSURE_DRAIN` (default off) — drain a run whose HEC target is failing to accept data. `STOKER_BACKPRESSURE_MIN_FAILED_FRACTION` (default 0.5) — the share of recent fleet delivery attempts (5xx + timeouts) that counts as the target struggling; `STOKER_BACKPRESSURE_SUSTAINED_S` (default 60) — how long that must hold before draining. See the supervisor-tick backpressure bullet.
