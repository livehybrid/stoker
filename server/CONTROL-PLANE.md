...
- **fleets**: id, name (unique), driver (swarm/k8s/inprocess/fake), config_json (portainer endpoint id + host; or k8s `namespace` + optional `kube_context` / `in_cluster`; or inprocess `max_workers` / `control_url`; or fake), version_info, last_seen_at, created_at. `fake-local` (fake), `swarm-local` (swarm) and `k8s-local` (k8s, namespace from `K8S_NAMESPACE`) are seeded at first boot from config, plus `inprocess-local` (inprocess) when `STOKER_INPROCESS_FLEET` is set; further fleets (e.g. a second cluster) are operator-added rows.
