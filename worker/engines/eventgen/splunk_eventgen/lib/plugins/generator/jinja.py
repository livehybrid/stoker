    ...
    def gen(self, count, earliest, latest, samplename=None):
        ...
        try:
            ...
            while self.current_count < self.target_count:
                ...
                # jinja2 3.x streams one chunk per output node instead of one
                # per template line, so render fully and split on lines
                rendered_template = jinja_loaded_template.render(jinja_loaded_vars)
                lines_out = []
                try:
                    for raw_line in rendered_template.splitlines():
                        ...
                        if line:
                            ...
                            try:
                                ...
                                logger.error(
                                    "Parse Failure Reason: {0}".format(str(e))
                                )
                                logger.error(
