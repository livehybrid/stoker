        ...
        if s.backfill is not None:
            ...
            if s.backfillSearch is not None:
                if s.backfillSearchUrl is None:
                    try:
                        # upstream referenced an undefined name `c` here
                        s.backfillSearchUrl = self.config.getSplunkUrl(s)[0]
                ...
                if s.backfillSearchUrl is not None:
                    ...
                    # stdlib replacement for the old httplib2 call, certificate
                    # validation disabled to match upstream behaviour
