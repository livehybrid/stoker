...
3. Poll `heartbeat` until a response carries `{"command":"release","t0":"<iso8601>"}` (a missing `t0` means "now"). Wait until the absolute wall-clock T0. The control plane always delivers `release` to a worker that has not yet started, **even if T0 is already in the past** (T0 elapses during a warm-up or heartbeat gap); the worker anchors its pacing at the shared T0 and catches up (bounded by `STOKER_CATCHUP_S`). It never receives `continue` in place of its one release, so a worker whose first post-T0 heartbeat lands late still starts rather than sitting warmed-but-paused at 0 EPS.
4. `POST heartbeat` every `telemetry.interval_s` with `{slot, lease_id, protocol_version, state, events_total, bytes_total, eps, hec_2xx, hec_4xx, hec_5xx, hec_timeouts, retries, dropped, queue_depth, lag_s, rss_mb, cpu_pct}` (`auth_failed: true` is added once HEC auth fails). Once the worker has sized its share it also sends the **optional** `assigned_work` (integer: how many discrete work units this slot actually holds — metrics: owned series; eventgen: stanzas that will emit; rawreplay: the one dataset) and, only when `assigned_work` is `0`, `assigned_reason` (a short human-readable explanation, e.g. "no series assigned: the pack's series matrix has 4 series and slot 7 of 10 workers owns none"). Both fields are additive and optional **in both directions**: a worker that predates them sends nothing and the control plane behaves as before; a control plane that predates them ignores the extra keys (its heartbeat model ignores unknown fields). The control plane persists them on the lease (as the private `_assigned_work` / `_assigned_reason` keys of the lease's stored share, exposed on the operator lease roster) so the UI can explain a slot that legitimately holds no work instead of showing a bare 0 EPS. Response `command` is one of `continue` | `release` | `retarget {share}` | `drain` | `superseded`. Any response may carry `jwt` for a rolling refresh (the agent replaces its bearer). An unknown command is logged and ignored.
...
4. `count_interval` mode: `count` split across workers by largest remainder using `slot`/`total_workers`; `interval` and everything else untouched. With more workers than a stanza's count, the surplus (high) slots receive a count of `0` for that stanza; a worker whose every stanza came out `0` holds no work at all and reports `assigned_work: 0` + an `assigned_reason` on its heartbeats (the control plane also refuses such an over-provisioned submit up front with `422 workers_exceed_shardable_work`).
